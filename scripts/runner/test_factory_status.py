...
class UtilizationTests(unittest.TestCase):
    ...
    def test_total_workers_includes_provider_child_slots(self):
        section = fs.utilization_section({'agents': {
            'codex-a': {'slots': 2}, 'codex-b': {'slots': 1},
            'claude': {'slots': 2},
        }}, 9, 2.5)
        self.assertEqual(section['total_workers'], 5)
        self.assertEqual(fs.configured_lane_keys({'agents': {
            'codex-a': {'slots': 2}, 'claude': {'slots': 2},
        }}), ['claude', 'claude-2', 'codex-a', 'codex-a-2'])
...
class BuildReportTests(unittest.TestCase):
    ...
    def test_child_lane_capacity_requires_explicit_green_eligibility(self):
        with tempfile.TemporaryDirectory() as d:
            state = pathlib.Path(d) / 'state'
            state.mkdir()
            for worker in ('codex-a', 'codex-a-2'):
                write_json(state / f'heartbeat-{worker}.json', {
                    'time': 1000.0, 'status': 'idle', 'worker': worker,
                })
            config = {'state': str(state), 'agents': {'codex-a': {'slots': 2}}}
            conservative = fs.capacity_status(state, config, 180, 1010.0)
            self.assertEqual(conservative['configured_lane_count'], 2)
            self.assertEqual(conservative['fresh_lane_count'], 2)
            self.assertEqual(conservative['dispatchable_lane_count'], 1)
            ...
            config['agents']['codex-a'].update({
                ...
                'fallback_model': 'luna', 'command': ['primary'],
                'fallback_command': ['fallback'],
            })
            write_json(state / 'usage.json', {'codex-a': {'account-a': {
                'provider': 'openai', 'model': 'astra', 'used_percent': 10,
                'observed_at': '1970-01-01T00:16:40Z',
            }}})
            green = fs.capacity_status(state, config, 180, 1010.0)
            self.assertEqual(green['dispatchable_lane_count'], 2)
            self.assertEqual(green['max_parallel_tasks'], 2)
