...
class MissingOrCorruptDataTests(unittest.TestCase):
    def test_issue_and_event_workers_are_loaded_with_legacy_agent_fallback(self):
        with tempfile.TemporaryDirectory() as d:
            state = pathlib.Path(d) / 'state'
            state.mkdir()
            write_json(state / 'issue-1.json', {
                'issue': 1, 'status': 'agent', 'time': 10.0,
                'agent': 'codex-a', 'worker': 'codex-a-2',
            })
            write_json(state / 'issue-2.json', {
                'issue': 2, 'status': 'agent', 'time': 20.0, 'agent': 'codex-a',
            })
            write_lines(state / 'events.jsonl', [
                {'timestamp': 10.0, 'status': 'agent',
                 'agent': 'codex-a', 'worker': 'codex-a-2'},
                {'timestamp': 20.0, 'status': 'agent', 'agent': 'codex-a'},
            ])
            records, corrupt = fd.load_issue_records(state)
            events = fd.load_events(state)
            self.assertEqual(corrupt, 0)
            self.assertEqual(records[1]['worker'], 'codex-a-2')
            self.assertEqual(records[2]['worker'], 'codex-a')
            self.assertEqual(events['events'][0]['worker'], 'codex-a-2')
            self.assertEqual(events['events'][1]['worker'], 'codex-a')
...
class UtilizationMathTests(unittest.TestCase):
    def test_child_workers_from_same_agent_count_as_concurrent_lanes(self):
        events_result = {'found': True, 'error': None, 'corrupt_lines': 0, 'events': [
            {'time': 0.0, 'status': 'agent', 'agent': 'codex-a', 'worker': 'codex-a'},
            {'time': 0.0, 'status': 'agent', 'agent': 'codex-a', 'worker': 'codex-a-2'},
        ]}
        result = fd.average_active_workers_for_window(
            events_result, now=100.0, window_start=0.0, window_end=100.0, total_workers=2)
        self.assertAlmostEqual(result['value'], 2.0)
        self.assertAlmostEqual(result['percent_of_capacity'], 1.0)

    def test_legacy_events_fall_back_to_agent_timeline(self):
        events_result = {'found': True, 'error': None, 'corrupt_lines': 0, 'events': [
            {'time': 0.0, 'status': 'agent', 'agent': 'codex-a'},
            {'time': 50.0, 'status': 'idle', 'agent': 'codex-a'},
        ]}
        result = fd.average_active_workers_for_window(
            events_result, now=100.0, window_start=0.0, window_end=100.0, total_workers=2)
...
class ProducerIntegrationTests(unittest.TestCase):
    def test_child_capacity_uses_same_staged_gate_as_runner(self):
        with tempfile.TemporaryDirectory() as d:
            state = pathlib.Path(d) / 'state'
            state.mkdir()
            for worker in ('codex-a', 'codex-a-2'):
                write_json(state / f'heartbeat-{worker}.json', {
                    'time': 990.0, 'status': 'idle', 'worker': worker,
                })
            config = {
                'state': str(state),
            ...
            def write_usage(percent):
                write_json(state / 'usage.json', {'codex-a': {'account-a': {
            ...
            disabled = fd.build_report('cfg.json', config_without_usage, now=1000.0)
            disabled_workers = {worker['key']: worker for worker in disabled['workers']}
            self.assertEqual(disabled['capacity']['dispatchable_lane_count'], 1)
            self.assertEqual(disabled_workers['codex-a-2']['dispatch_decision'], 'defer')

    def test_build_report_expands_slot_capacity_and_worker_utilization(self):
        with tempfile.TemporaryDirectory() as d:
            state = pathlib.Path(d) / 'state'
            state.mkdir()
            write_lines(state / 'events.jsonl', [
                {'timestamp': 0.0, 'status': 'agent',
                 'agent': 'codex-a', 'worker': 'codex-a'},
                {'timestamp': 0.0, 'status': 'agent',
                 'agent': 'codex-a', 'worker': 'codex-a-2'},
            ])
            ...
            report = fd.build_report('cfg.json', config, now=100.0)
            self.assertEqual(report['utilization']['total_workers'], 2)
            self.assertAlmostEqual(report['utilization']['total']['value'], 2.0)
            self.assertAlmostEqual(report['utilization']['total']['percent_of_capacity'], 1.0)
            self.assertEqual([worker['key'] for worker in report['workers']],
                             ['codex-a', 'codex-a-2'])
            self.assertTrue(all(worker['provider'] == 'openai'
...
class WorkerStateTests(unittest.TestCase):
    def test_child_lane_uses_own_records_and_parent_dispatch_metadata(self):
        agent_defs = fd.agent_definitions({'agents': {'codex-a': {
            'provider': 'openai', 'model': 'astra', 'label': 'Agent A', 'slots': 2,
        }}})
        records = {
            1: {'issue': 1, 'status': 'review', 'time': 100.0, 'agent': 'codex-a',
                'worker': 'codex-a', 'branch': None, 'commit': None, 'pr': None,
                'error': None, 'diff_stat': None},
            2: {'issue': 2, 'status': 'agent', 'time': 200.0, 'agent': 'codex-a',
        ...
        workers = fd.build_worker_views(
            agent_defs, records, events=[], heartbeat={}, now=300.0,
            dispatch_models=projected,
        )
        by_key = {worker['key']: worker for worker in workers}
        child = by_key['codex-a-2']
        self.assertEqual(child['agent'], 'codex-a')
        self.assertEqual(child['slot'], 2)
        self.assertEqual(child['provider'], 'openai')
        self.assertEqual(child['model'], 'astra')
        self.assertEqual(child['effective_model'], 'astra')
        self.assertEqual(child['current_issue'], 2)
...
class AgentDefinitionsTests(unittest.TestCase):
    def test_slots_expand_child_worker_keys_with_parent_metadata(self):
        config = {'agents': {'codex-a': {
            'provider': 'openai', 'model': 'astra', 'label': 'Agent A', 'slots': 3,
        }}}
        defs = fd.agent_definitions(config)
        self.assertEqual(list(defs), ['codex-a', 'codex-a-2', 'codex-a-3'])
        self.assertEqual(defs['codex-a'], {
            'key': 'codex-a', 'agent': 'codex-a', 'slot': 1,
            'provider': 'openai', 'model': 'astra', 'label': 'Agent A',
        })
        self.assertEqual(defs['codex-a-3'], {
            'key': 'codex-a-3', 'agent': 'codex-a', 'slot': 3,
            'provider': 'openai', 'model': 'astra', 'label': 'Agent A',
        })

    def test_invalid_slot_counts_do_not_expand_capacity(self):
        for slots in (0, 4, True, '3'):
            with self.subTest(slots=slots):
                defs = fd.agent_definitions({'agents': {'codex-a': {'slots': slots}}})
                self.assertEqual(list(defs), ['codex-a'])
