...
from scripts.factory_registry.models import (  # noqa: E402
    Evidence, ReviewInput, ReviewOutcome, TaskStatus,
)
...
class RunnerRegistryControl:
    ...
    def record_review_input(self, review_input: ReviewInput) -> None:
        """Persist the exact implementation state before a review can claim it."""
        self.registry.record_review_input(review_input, operation_id=f"review-input:{review_input.id}")

    def record_implementation_review_inputs(self, *, target_package_id: str, implementation_attempt_id: str, implementation_commit: str, base_commit: str, pr_url: str, contract: dict, validation_evidence: dict) -> tuple[str, ...]:
        """Bind every registered dependent review to this immutable result."""
    ...
    def record_review_outcome(self, outcome: ReviewOutcome, evidence: Evidence, *, expected_revision: int) -> int:
        """Only a structured Registry verdict may close a review."""
