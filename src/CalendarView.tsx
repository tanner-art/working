...
import { addDays, addMonths, buildCalendarMonth, buildCalendarTimeGrid, dayAriaLabel, dayNumber, monthLabel, parseLocalDateKey, weekStart, WEEKDAY_LABELS } from './calendar'
...
export function CalendarView({ state, onOpen, onUpdate }: { state: AppState; onOpen: (id: string) => void; onUpdate: (next: (state: AppState) => AppState) => void }) {
  ...
  const go = (direction: -1 | 1) => {
    if (mode === 'month') {
      const next = addMonths(selectedDate.getFullYear(), selectedDate.getMonth(), direction)
      const day = Math.min(selectedDate.getDate(), new Date(next.year, next.month + 1, 0).getDate())
      setSelected(localDateKey(new Date(next.year, next.month, day)))
    } else setSelected(addDays(selected, direction * (mode === 'week' ? 7 : 1)))
  }
...
function TimeGrid({ days, onOpen }: { days: CalendarDayCell[]; onOpen: (id: string) => void }) {
  ...
  return <div className="time-grid-wrap" role="grid" aria-labelledby="calendar-view-label"><div className="time-grid-head" style={columns}><span aria-hidden="true" />{days.map(day => <span key={day.date} className={day.isToday ? 'is-today' : ''}>{dateLabel(day.date)}</span>)}</div><div className="time-grid" style={columns}><div className="time-labels">{hours.map(hour => <span key={hour}>{hour === 0 ? '' : new Intl.DateTimeFormat('en-US', { hour: 'numeric' }).format(new Date(2026, 0, 1, hour))}</span>)}</div>{days.map(day => <div key={day.date} className={`time-column ${day.isToday ? 'is-today' : ''}`}>{hours.map(hour => <div key={hour} className="time-slot" />)}{day.proposedDates.map(marker => <button type="button" key={marker.interpretationId} className="time-proposed" onClick={() => onOpen(marker.objectId)}>{marker.summary}</button>)}{day.events.map(({ event, linkedObjects }) => { const start = new Date(event.startsAt); const position = (start.getHours() * 60 + start.getMinutes()) / 60; return <div key={event.id} className="time-event" style={{ top: `${position * 52}px` }}><strong>{event.title}</strong><small>{time(event.startsAt)}</small>{linkedObjects.map(object => <button key={object.id} type="button" onClick={() => onOpen(object.id)}>{objectLabels[object.kind]}: {object.summary}</button>)}</div> })}{day.date === nowKey && <span className="current-time" style={{ top: `${(now.getHours() * 60 + now.getMinutes()) / 60 * 52}px` }} aria-label="Current time" />}</div>)}</div></div>
}
