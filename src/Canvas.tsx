...
function ColorControl({ label, value, colors, onChange }: { label: string; value: string; colors: readonly string[]; onChange: (value: string) => void }) {
  return <fieldset className="canvas-color-control"><legend>{label}</legend><div className="canvas-swatches">
    {colors.map(color => <button type="button" key={color} className={value.toLowerCase() === color.toLowerCase() ? 'selected' : ''}
...
export function Canvas({ title, autoFocusTitle, elements, viewport, onTitle, onViewport, onCommit, onText, onFinishText, canUndo, canRedo, onUndo, onRedo, onCaptureObject, onExit, saveStatus }: {
  ...
  const shapePreview = (stroke: CanvasElement) => previewCanvasStrokeShape(stroke.rawPoints)
  const shapePreviewPoints = (stroke: CanvasElement) => { const projection = shapePreview(stroke); return projection && projectCanvasStroke(stroke.rawPoints, projection) }
  ...
  const acceptShape = () => {
    const stroke = refinementCandidateId ? elements.find(item => item.id === refinementCandidateId && item.type === 'freehand') : undefined
    if (!stroke || stroke.projection !== undefined) { dismissRefinementPreview(); return }
    const shaped = applyCanvasStrokeShape(stroke, new Date().toISOString())
    if (!shaped) { dismissRefinementPreview(); return }
    onCommit(elements.map(item => item.id === stroke.id ? shaped : item))
    dismissRefinementPreview()
  }
  ...
  return <div className="canvas-page">
    ...
    </label>)}<ColorControl label="Shape color" value={selectedElement.fillColor ?? (selectedElement.type === 'container' ? DEFAULT_CONTAINER_COLOR : DEFAULT_CANVAS_COLOR)} colors={CANVAS_COLORS} onChange={fillColor => onCommit(updateCanvasNodeAppearance(elements, selectedElement.id, fillColor))} /></>}{selectedElement?.type === 'arrow' && <>
    ...
    {refinementCandidateId && <section className="canvas-refinement-sheet" role="dialog" aria-modal="false" aria-labelledby="smoothing-preview-title">
      <button className="canvas-refinement-dismiss" aria-label="Dismiss refinement preview" onClick={dismissRefinementPreview}>×</button>
      <p className="section-label">Local refinement preview</p><h2 id="smoothing-preview-title">Refine this stroke?</h2>
      <p>The highlighted line is a deterministic local preview. Its original raw points stay preserved.</p>
      <div><button className="secondary" onClick={dismissRefinementPreview}>Keep original</button><button className="primary" onClick={acceptSmoothing}>Use smoothed stroke</button>{(() => { const stroke = elements.find(item => item.id === refinementCandidateId && item.type === 'freehand'); const proposal = stroke && shapePreview(stroke); return proposal && <button className="primary" onClick={acceptShape}>Convert to {proposal.shape}</button> })()}</div>
    </section>}
    </div>
}
