...
export function interpret(content: string): { kind: ObjectKind; confidence: number; interpretation: Interpretation } {
  ...
  if (/\b(remind me|reminder|remember to)\b/.test(value)) { kind = 'reminder'; confidence = .74; rationale = 'This asks for a reminder; confirm the timing before scheduling it.' }
