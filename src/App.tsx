  ...
  return <div className="panel-backdrop" onMouseDown={onClose}>
    <aside className="object-panel" onMouseDown={event => event.stopPropagation()}>
      ...
      <div className="panel-scroll">
        ...
        {object.source === 'text' && <section className="thought-editors" aria-labelledby="thought-editing-heading"><div><p className="section-label">Edit without losing history</p><h3 id="thought-editing-heading">Revise this thought</h3><p>Each save adds a version. The original capture always stays available below.</p></div>
          ...
          <label>Thought text<textarea value={correction} onChange={event => setCorrection(event.target.value)} /></label>
          <button className="secondary" disabled={hasUnsavedFieldEdits || !correction.trim() || correction.trim() === history.currentText} onClick={() => { if (window.confirm('Save this as the current thought text while keeping the original capture?')) onCorrect(correction) }}>Save text version</button>
