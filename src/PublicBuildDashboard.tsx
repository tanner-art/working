...
import { capacitySummary, effectiveWorkerHealth, emptyQueueFilters, filterFactoryFeatures, packageIndex, visibleAttention, visibleReviews, type QueueFilters } from './buildDashboard'
...
function WorkerSummary({ worker, item, scopes, generatedAt }: { worker: FactoryWorker; item?: FactoryPackage; scopes: FactoryCapacityScope[]; generatedAt: string }) {
  ...
  return <article className="factory-card worker-summary"><div className="factory-card-heading"><div className="factory-avatar" aria-hidden="true">{worker.displayName.slice(0, 2).toUpperCase()}</div><div><h3>{worker.displayName}</h3><p>{worker.role === 'ORCHESTRA' ? 'Coordinator' : worker.provider && worker.model ? `${worker.provider} · ${worker.model}` : worker.role}</p></div><StatusChip label={health.state} tone={health.state} /></div>
...
function Failures({ snapshot }: { snapshot: FactoryControlSnapshot }) {
  ...
  return <div className="factory-view"><section className="factory-section"><SectionHeading title="Failures requiring attention" note="Authentication, heartbeat, capacity, scope, CI, review, dependency and provenance issues" />{failures.length ? <div className="factory-failure-list">{failures.map(failure => <article className={`factory-card factory-failure ${failure.severity}`} key={failure.id}><div className="factory-card-heading"><div><span className="factory-id">{failure.code}</span><h2>{failure.title}</h2></div><StatusChip label={failure.requiresHuman ? 'Human action needed' : failure.source === 'package_state' ? 'Remediation needed' : failure.severity} tone={failure.severity === 'critical' ? 'danger' : 'warning'} /></div>{failure.source === 'package_state' && <p className="factory-scope-line">Derived from this package’s Registry state because no matching structured failure row is present.</p>}<p>{failure.detail}</p><div className="factory-event-path"><span>{workerName(failure.workerId)}</span><span>{failure.packageId ?? 'No package'}</span><span>{failure.occurredAt ? formatRelativeTime(failure.occurredAt) : 'Time not recorded'}</span></div></article>)}</div> : <Empty title="No failures need attention" text="The current Registry projection contains no active intervention items." />}</section></div>
}
