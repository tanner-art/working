...
The Registry projection reconciles sparse structured rows against package state from that same revision. Every `VERIFY_REVIEW` package remains visible in Reviews even when the Registry has no corresponding structured review row. Final independent outcomes are first-class append-only Registry records linked to the review package, target package, Registry-derived implementer, reviewer-owned review attempt, timing, findings, requested changes, and typed review evidence structurally bound to that attempt. A reviewer who attempted the target implementation, an implementer claim that differs from the latest successful target attempt, any target attempt that overlaps the review interval, or approval evidence without the reviewer attempt binding is rejected. Overlap includes an attempt that began before review was requested but completed afterward or remained active. A missing review outcome is displayed as neutral **Outcome unrecorded**, never as waiting, approved, or rejected. A package-level failure code or `changes_requested` review remains visible in Failures / attention when the structured failure list omits it. When a `VERIFY_REVIEW` package has typed `review` evidence but no structured review outcome or current failure anywhere in that revision, the projection authoritatively emits a current `REVIEW_STATE_UNRECORDED` warning, increments attention, and constrains aggregate Factory health. Historical failures with `requiresHuman: false` do not suppress that current reconciliation item. Neither the projector nor the browser inspects the human evidence label or infers whether the review approved or rejected the package. The browser retains the same conservative derivation for compatible sparse snapshots, and labels unavailable reviewer, timing, or failure fields as not recorded.
...
Each Work Package carries its Registry kind, attempts, and attempt-specific branch, commit, pull request, and evidence projection. Package kind lets both validators recompute the active parent total from ACTIVE `PARENT` packages in the same revision. The History drill-through uses those fields directly. Event history remains a separate chronological view of Registry transitions; neither surface queries Git or GitHub from the browser.
