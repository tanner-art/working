...
Only structured failures with `requiresHuman: true` appear in Failures / attention or contribute to the authoritative Overview attention count. Resolved observations with `requiresHuman: false` remain available under History / provenance and do not constrain aggregate Factory health merely because their historical severity was critical.
